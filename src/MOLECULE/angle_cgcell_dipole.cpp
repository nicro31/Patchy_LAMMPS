...
#include "error.h"
  ...
  /////////////////////////////////////////////////
  /// Replace dipole by ellipsoid orientation NR///
  /////////////////////////////////////////////////
...
AngleCgcellDipole::AngleCgcellDipole(LAMMPS *lmp) : Angle(lmp)
{
  k = NULL;
  gamma0 = NULL;
}
...
AngleCgcellDipole::~AngleCgcellDipole()
{
  if (allocated) {
...
void AngleCgcellDipole::compute(int eflag, int vflag)
{
  ...
  // Declare additionnal variables
  double mu[3][4];
  double a1[3][3];
  double *iquat;
  // Choose ellipsoid axis to orient (body frame)
  double dipOrient[3]; dipOrient[0] = 1.0; dipOrient[1] = 0.0; dipOrient[2] = 0.0;
  double dipOrientRotated[3]; // dipOrient in simulation box frame
  // Get atoms ellipsoid parameters
  avec = (AtomVecEllipsoid *) atom->style_match("ellipsoid");
  if (!avec) error->all(FLERR,"Angle cgcell dipole requires atom style ellipsoid");
  ...
  /// Replace dipole by ellipsoid orientation NR///
  ///////////////////////////////////////////////// 
    // replace dipole orientation by [1 0 0] ellipsoid orientation
    iquat = bonus[ellipsoid[iDip]].quat;
    MathExtra::quat_to_mat_trans(iquat,a1);
    MathExtra::matvec(a1, dipOrient,dipOrientRotated);
    mu[iDip][0] = dipOrientRotated[0];
    mu[iDip][1] = dipOrientRotated[1];
    mu[iDip][2] = dipOrientRotated[2];
    mu[iDip][3] = 1.0;
...
void AngleCgcellDipole::allocate()
{
  allocated = 1;
...
void AngleCgcellDipole::coeff(int narg, char **arg)
{
...
double AngleCgcellDipole::equilibrium_angle(int i)
{
  return gamma0[i];
}
...
void AngleCgcellDipole::write_restart(FILE *fp)
{
...
void AngleCgcellDipole::read_restart(FILE *fp)
{
  allocate();
...
void AngleCgcellDipole::write_data(FILE *fp)
{
...
double AngleCgcellDipole::single(int type, int iRef, int iDip, int /*iDummy*/)
{
  double **x = atom->x; // position vector
