...
enum{DIST,VELVIB,OMEGA,ENGTRANS,ENGVIB,ENGROT,ENGPOT,FORCE,VARIABLE,ANGLE,ANGLE1,ANGLE2,DIHEDRAL};
  ...
  bstyle(NULL), vvar(NULL), dstr(NULL), vstr(NULL), vlocal(NULL), alocal(NULL)
{
  ...
  for (iarg = 3; iarg < narg; iarg++) {
    ...
    else if (strcmp(arg[iarg],"angle") == 0) { bstyle[nvalues++] = ANGLE; ellipsoidStyle = true; }
    else if (strcmp(arg[iarg],"angle1") == 0) { bstyle[nvalues++] = ANGLE1; ellipsoidStyle = true; }
    else if (strcmp(arg[iarg],"angle2") == 0) { bstyle[nvalues++] = ANGLE2; ellipsoidStyle = true; }
    else if (strcmp(arg[iarg],"dihedral") == 0) { bstyle[nvalues++] = DIHEDRAL; ellipsoidStyle = true; }
...
int ComputeBondLocal::compute_bonds(int flag)
{
  ...
  for (atom1 = 0; atom1 < nlocal; atom1++) {
    ...
    for (i = 0; i < nb; i++) {
      ...
      /// NR, add for coarse grained ///
      if(ellipsoidStyle)
      {
	// NR, Calculate angle 1 and angle 2, and angle
	qa=bonus[atom1].quat;
	...
	bx[2] = -bx[2];
	rmu = sqrt(rsq);
	angle1 = acos( (bx[0]*dx+bx[1]*dy+bx[2]*dz) / rmu ) * 180.0 / pi_cst;
	angle2 = acos( (-ax[0]*dx-ax[1]*dy-ax[2]*dz) / rmu ) * 180.0 / pi_cst;
	angle  = acos(ax[0]*bx[0] + ax[1]*bx[1] + ax[2]*bx[2]) * 180.0 / pi_cst;
	
	// NR, calculate dihedral angle
	rAB[0] = dx; 
	rAB[1] = dy;
	rAB[2] = dz;
	...
	if(lambdaD < -1.0)
	  lambdaD = -1.0;
	dihedral = (gammaD * acos(lambdaD)) * 180.0 / pi_cst;
      }
