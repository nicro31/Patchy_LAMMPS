  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype] && factor_lj != 0) // NR try add not calculate if factor_lj = 0
      {
        ...
        bool patch_j_done[patchInter[itype][jtype].nbP2];
	for(int l(0) ; l < patchInter[itype][jtype].nbP2 ; l++)
	{
	  patch_j_done[l] = false;
	}
      ...
      if(costi_a >= pGeometry[patchInter[itype][jtype].typeP1[k]].cosThetaMax_a && costi_b >= pGeometry[patchInter[itype][jtype].typeP1[k]].cosThetaMax_b)
      {
		  ...
		  rij_b_norm_vect[l] = rij_b_norm;
			...
			*/
                    ...
                    // Compute torque on i
                    MathExtra::cross3(ni, rij, v1);
                    MathExtra::cross3(ni, rij, v2);
                    MathExtra::cross3(ai, rij, v3);
                    MathExtra::cross3(bi, rij, v4);
                    for(int m(0) ; m < 3 ; m++)
                    {
                        delti[m] +=  morseFactor *
                        (
                         -v1[m] * prefacti_a * cosFactori_b * cosFactorj_a * cosFactorj_b *  rji_sca_ai * rji_sca_ai / rji_a_norm_cube 
                         -v2[m] * prefacti_b * cosFactori_a * cosFactorj_a * cosFactorj_b * rji_sca_bi * rji_sca_bi / rji_b_norm_cube +
                         v3[m] * prefacti_a * cosFactori_b * cosFactorj_a * cosFactorj_b * ni_sca_rij * (-rji_sca_ai) / rji_a_norm_cube +
                         v4[m] * prefacti_b * cosFactori_a * cosFactorj_a * cosFactorj_b * ni_sca_rij * (-rji_sca_bi) / rji_b_norm_cube
                        );
                    }

                    // Compute torque on j
                    MathExtra::cross3(nj, rij, v1);
                    MathExtra::cross3(nj, rij, v2);
                    MathExtra::cross3(aj, rij, v3);
                    MathExtra::cross3(bj, rij, v4);
                    for(int m(0) ; m < 3 ; m++)
                    {
                        deltj[m] +=  morseFactor *
                        (
                         v1[m] * prefactj_a * cosFactorj_b * cosFactori_a * cosFactori_b * rij_sca_aj * rij_sca_aj / rij_a_norm_cube +
                         v2[m] * prefactj_b * cosFactorj_a * cosFactori_a * cosFactori_b * rij_sca_bj * rij_sca_bj / rij_b_norm_cube +
                         v3[m] * prefactj_a * cosFactorj_b * cosFactori_a * cosFactori_b * nj_sca_rij * (-rij_sca_aj) / rij_a_norm_cube +
                         v4[m] * prefactj_b * cosFactorj_a * cosFactori_a * cosFactori_b * nj_sca_rij * (-rij_sca_bj) / rij_b_norm_cube
                        );
                    }
